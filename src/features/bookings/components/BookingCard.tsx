...
const BookingCard = ({
  ...
  return (
    <article className="min-w-0 rounded-xl border border-gray-200 bg-white p-4 shadow-sm transition hover:shadow-md sm:p-5">
      {/* Header */}
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <span className="w-fit shrink-0 rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold capitalize text-gray-700">
          {booking.status}
        </span>
      </div>
        ...
        {booking.selectedBeds.length > 0 ? (
          <div className="mt-2 flex flex-wrap gap-2">
            {booking.selectedBeds.map((bed) => (
              <span
                key={bed.type}
                className="rounded-lg bg-gray-100 px-3 py-1.5 text-sm capitalize text-gray-700"
              >
      ...
      {/* Actions */}
      <div className="mt-5 flex gap-2 border-t border-gray-200 pt-4 sm:justify-end">
        <button
          type="button"
